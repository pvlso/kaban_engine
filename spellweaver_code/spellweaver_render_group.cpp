...
PushRenderElement_(render_group *Group, uint32 Size, render_group_entry_type Type, r32 SortKey)
{
    ...
    {
        Platform.WriteLogFile(L"Error: Render push buffer owerflow", __FILE__, __LINE__);
        InvalidCodePath;
    }
...
inline loaded_quest *
PushQuest(render_group *Group, quest_id ID, b32 Immidiate = false)
{
    loaded_quest *Quest = GetQuest(Group->Assets, ID, Group->GenerationID);
    if((Group->RendersInBackground || Immidiate) && !Quest)
    {
        LoadQuest(Group->Assets, ID, true);
        Quest = GetQuest(Group->Assets, ID, Group->GenerationID);
    }
    ...
    {
        Assert(!Group->RendersInBackground);
        LoadQuest(Group->Assets, ID, false);
        ++Group->MissingResourceCount;
    }
...
PushClipRect(render_group *Group, object_transform ObjectTransform, v3 Offset, v2 Dim)
{
    ...
    if(Basis.Valid)
    {
        ...
        Result = PushClipRect(Group, 
            RoundReal32ToInt32(BasisP.x), RoundReal32ToInt32(BasisP.y),
