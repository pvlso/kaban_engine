...
internal void
LoadBinaryFile(game_assets *Assets, file_id ID, b32 Immediate)
{
    TIMED_FUNCTION();
...
internal void
LoadQuest(game_assets *Assets, quest_id ID, b32 Immediate)
{
    asset *Asset = Assets->Assets + ID.Value;
    if(ID.Value)
    {
           ...
           AssetState_Unloaded)
        {
            ...
            if(Immediate || Task)
            {
                ssa_quest *Info = &Asset->SSA.Quest;

                u32 TextIDsSize = sizeof(text_id)*(Info->GiverParCount +
                                                   Info->ObjectiveParCount +
                                                   Info->ComplitionParCount);
                u32 SizeData = TextIDsSize;
                u32 SizeTotal = SizeData + sizeof(asset_memory_header);

                Asset->Header = AcquireAssetMemory(Assets, SizeTotal, ID.Value, AssetType_Quest);

                loaded_quest *Quest = &Asset->Header->Quest;
                Quest->TextIDOffset = GetFile(Assets, Asset->FileIndex)->AssetTypeOffsets[Asset_Text];
                Quest->TextIDs = (text_id *)(Asset->Header + 1);
                ...
                Work.Offset = Asset->SSA.DataOffset;
                Work.Size = SizeData;
                Work.Destination = Quest->TextIDs;
                Work.FinalizeOperation = FinalizeAsset_None;
                Work.FinalState = AssetState_Loaded;
...
internal void
LoadText(game_assets *Assets, text_id ID, b32 Immediate)
{
    asset *Asset = Assets->Assets + ID.Value;
    if(ID.Value)
    {
...
inline text_id
GetBestMatchTextFrom(game_assets *Assets, asset_type_id TypeID, asset_vector *MatchVector, asset_vector *WeightVector)
{
    text_id Result = {};
    Result.Value = GetBestMatchAssetFrom(Assets, TypeID, MatchVector, WeightVector);
...
inline quest_id
GetBestMatchQuestFrom(game_assets *Assets, asset_type_id TypeID, asset_vector *MatchVector, asset_vector *WeightVector)
{
    quest_id Result = {};
    Result.Value = GetBestMatchAssetFrom(Assets, TypeID, MatchVector, WeightVector);
...
inline file_id
GetBestMatchFileFrom(game_assets *Assets, asset_type_id TypeID, asset_vector *MatchVector, asset_vector *WeightVector)
{
    file_id Result = {};
    Result.Value = GetBestMatchAssetFrom(Assets, TypeID, MatchVector, WeightVector);
...
AllocateGameAssets(memory_arena *Arena, memory_index Size, transient_state *TranState)
{
    ...
    if(AssetCount == Assets->AssetCount)
    {
        Platform.WriteLogFile(L"Game Assets Allocated", __FILE__, __LINE__);
    }
    else
    {
        Platform.WriteLogFile(L"Error: Unable to allocate game assets", __FILE__, __LINE__);
    }
...
inline sound_id
GetSoundEffectForType(game_assets *Assets, sound_effect_type Type, u32 Variety = 0)
{
    sound_id Result = {};

    asset_vector WeightVector = {};
    WeightVector.E[Tag_Variety] = 1.0f;
    WeightVector.E[Tag_SoundEffectType] = 1.0f;

    asset_vector MatchVector = {};
    MatchVector.E[Tag_Variety] = (r32)Variety;
    MatchVector.E[Tag_SoundEffectType] = (r32)Type;
...
internal text_id
GetQuestTextIDForNPC(loaded_quest *Quest, ssa_quest *QuestInfo, talkingnpc_entity *NPCData)
{
    u32 TextCount = (QuestInfo->GiverParCount +
                     QuestInfo->ObjectiveParCount +
                     QuestInfo->ComplitionParCount);

    Assert(NPCData->ParagraphIndex < TextCount);
    u32 ParagraphIndex = 0;
    switch(NPCData->TalkingState)
    {
        case TalkingState_QuestGiver:
        {
            Assert(NPCData->ParagraphIndex < TextCount);
            ParagraphIndex = NPCData->ParagraphIndex;
        } break;

        case TalkingState_QuestObjective:
        {
            Assert((NPCData->ParagraphIndex + QuestInfo->GiverParCount) < TextCount);
            ParagraphIndex = NPCData->ParagraphIndex + QuestInfo->GiverParCount;
        } break;

        case TalkingState_QuestComleted:
        {
            Assert((NPCData->ParagraphIndex + (QuestInfo->GiverParCount +
                                               QuestInfo->ObjectiveParCount)) < TextCount);
    ...
    text_id Result = Quest->TextIDs[ParagraphIndex];
    Result.Value += Quest->TextIDOffset;
