...
;; Define a custom face for specific types
;(defface font-lock-custom-type-face
;  '((t :foreground "DarkGoldenrod3"))
;  "Face for custom types.")

;; Function to add custom font-lock rules
;(defun add-custom-type-highlighting ()
;  (font-lock-add-keywords
;   nil
;   '(("\\b\\(u8\\|u16\\|u32\\|u64\\|s32\\|b32\\|int\\|float\\|bool\\|double\\|int8\\|int16\\|int32\\|int64\\|uint8\\|uint16\\|uint32\\|uint64\\)\\b" . 'font-lock-custom-type-face))))

;; Define custom faces
(defface font-lock-decimal-face
  '((t :foreground "olive drab"))
  "Face for decimal numbers.")

(defface font-lock-float-face
  '((t :foreground "olive drab"))
  "Face for floating-point numbers.")

(defface font-lock-hex-face
  '((t :foreground "olive drab"))
  "Face for hexadecimal numbers.")

;; Function to add custom font-lock rules
(defun add-custom-number-highlighting ()
  (font-lock-add-keywords
   nil
   '(("\\b\\([0-9]+\\.[0-9]*\\|[0-9]*\\.[0-9]+\\)\\([eE][-+]?[0-9]+\\)?[fFlL]?\\b" . 'font-lock-float-face) ; Floating-point numbers, including exponent and suffix
     ("\\b0x[0-9a-fA-F]+\\b" . 'font-lock-hex-face)                ; Hexadecimal numbers
     ("\\b\\([0-9]+\\)\\b" . 'font-lock-decimal-face))))           ; Decimal numbers
...
(add-hook 'c-mode-hook 'add-custom-number-highlighting t)
(add-hook 'c++-mode-hook 'add-custom-number-highlighting t)
