...
{
    return pcg_rotr_8((u8)(((state >> 5u) ^ state) >> 5u), state >> 13u);
}

inline uint16_t pcg_output_xsh_rr_32_16(uint32_t state)
{
    return pcg_rotr_16((u16)(((state >> 10u) ^ state) >> 12u), state >> 28u);
}

inline uint32_t pcg_output_xsh_rr_64_32(uint64_t state)
{
    return pcg_rotr_32((u32)(((state >> 18u) ^ state) >> 27u), state >> 59u);
}
