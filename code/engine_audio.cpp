...
//    TIMED_FUNCTION();
    ...
    if(IsValid(SoundID) || Sound)
    {
        ...
        Result = AudioState->FirstFreePlayingSound;
        AudioState->FirstFreePlayingSound = Result->Next;

        Result->Terminated = false;
        Result->SoundIsPlaying = true;
        Result->SamplesPlayed = 0;
        // TODO(casey): Should these default to 0.5f/0.5f for centerred?
        Result->CurrentVolume = Result->TargetVolume = V2(0.4f, 0.4f);
        Result->dCurrentVolume = V2(0, 0);
        Result->Sound = Sound;
        Result->ID = SoundID;
        Result->dSample = 1.0f;

        Result->Next = AudioState->FirstPlayingSound;
...
internal void
MuteAndTerminateSound(audio_state *AudioState, playing_sound *Sound, real32 FadeDurationInSeconds)
{
    if(Sound)
    {
        if(!Sound->Terminated)
        {
            if(FadeDurationInSeconds <= 0.0f)
            {
                Sound->CurrentVolume = Sound->TargetVolume = V2(0.0f, 0.0f);
            }
            else
            {
                real32 OneOverFade = 1.0f / FadeDurationInSeconds;
                Sound->TargetVolume = V2(0.0f, 0.0f);
                Sound->dCurrentVolume = OneOverFade*(Sound->TargetVolume - Sound->CurrentVolume);
            }

            if((Sound->CurrentVolume.x <= 0.01f) && (Sound->CurrentVolume.y <= 0.01f))
            {
...
OutputPlayingSounds(audio_state *AudioState,
                    editor_sound_output_buffer *SoundBuffer, editor_assets *Assets,
...
#define AudioStateOutputChannelCount 2
        ...
        )
    {
        ...
        bool32 SoundFinished = PlayingSound->Terminated;
        ...
        if(SoundFinished)
        {
            *PlayingSoundPtr = PlayingSound->Next;
            ...
            PlayingSound->SoundIsPlaying = false;
        }
        else
        {
            PlayingSoundPtr = &PlayingSound->Next;
